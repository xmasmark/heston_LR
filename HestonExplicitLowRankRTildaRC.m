function MU = HestonExplicitLowRankRTildaRC(params,K,r,q,S,V,T)
...
%testMatrix = U-X*Y';
    ...
    for t=1:NT-1
        ...
        %two boundary conditions applied
        %on the X matrix. The first is about
        %the first row, the last is about
        %the last row of the X matrix
        ...
        for j = 1:records
            ...
            %product between X and the last column of Y'
            %because that's the only relevant
            for i = 1:fields-1
                acc = acc + x(j,i)*y(NV,i);
                %x(1,i)=0; %do this later please
        ...
        %the last column of Y needs to give S
        % for i = 1:fields
        %     y(records,i)=S ;
        %     %x(records,i)=max(0,Smax-K);
        % end
        ...
        sdX = MDerivative(fdX,ds);  %second derivative of X
        sdY = MDerivative(fdY,dv);  %second derivative of Y
        ...
        %thetaElement = PWProduct((theta-V),fdY);
        ...
        % %with the low rank Tilda approach this part is not needed
        % %anymore. Furthermore, it doesn't even work.
        ...
        %I try to add here the boundary conditions.
        %%here boundary conditions start
        % % % % % xones = ones(NS,1);
        % % % % % yzeros = zeros(NV,1);
        % % % % % yzeros(NV)=1;
        % % % % % 
        ...
        % % % % %     %because that's the only relevant
        % % % % %     for i = 1:fields-1
        % % % % %         acc = acc + newXBC(j,i)*newYBC(NV,i);
        % % % % %         %x(1,i)=0; %do this later please
        % % % % %         %x(records,i)=x(records-1,i)+(ds/(fields));
        % % % % %     end
        % % % % %     newXBC(j,fields)=S(j)-acc;
        % % % % % end
        % % % % % 
        % % % % % for i = 1:fields-1
        % % % % %     newXBC(1,i)=0;
        ...
        % % % % %     %x(records,i)=x(records-1,i)+(ds/(fields));
        % % % % % end
        %%here boundary conditions finish
        % % % % % 
        % % % % % MU = newX*newY';
        ...
        % % % % % [X,Y]=CompressData(newXBC,newYBC,epsilon);
