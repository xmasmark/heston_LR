function U = HestonExplicitClassicCNALSDev10(params,K,r,q,S,V,T, mode, iterations, restart, cnIterations)
    ...
    for t = 1:NT-1
        ...
        [xALS,yALS]=ALSOptimizationW(Ap, Bp, xALS, yALS, BXc, BYc, epsilon, max_iter, restart, cnIterations);        
...
function [X, Y] = ALSOptimizationW(A, B, x, y, BX, BY, epsilon, max_iter, restart, cnIterations)
...
function [X, Y, nn] = ALSOptimizationV04(A, B, x, y, BXc, BYc, epsilon, max_iter, restart)
    ...
    if r >= 2
        ...
        %A_hatP = permute(A_hat,[1,3,2,4]); %this works
        A_hatP = permute(A_hat,[2,3,1,4]);
        A_hat_matrix = reshape(A_hatP,NS*r,NS*r);
        %b_hat_vector = reshape(b_hat,NS*r,1); %this works
