function MU = HestonExplicitLowRankGMRES(params,K,r,q,S,V,T,epsilon)
    ...
    for t=1:NT-1
        ...
        sdX = MSecondDerivative(x,ds, boundaryType);  %second derivative of X
        sdY = MSecondDerivative(y,dv, boundaryType);  %second derivative of Y
...
function D = MSecondDerivative_without_method(L, delta)
    % Calculate the size of the input matrix L
    [records, fields] = size(L);
...
function D = MSecondDerivative(matrix, h, method)
    % Calculate the size of the input matrix 
    [n, fields] = size(matrix);
    ...
    % Adjust the boundary conditions based on the method
    if method == 0
        % Dirichlet boundary conditions (already implemented)
        laplacian(1, 1) = 1;
        laplacian(1, 2) = -2;
        laplacian(1, 3) = 1;
        laplacian(n, n-2) = 1;
    ...
    elseif method == 1
        ...
        error('Invalid method. Use 0 for Dirichlet or 1 for Neumann boundary conditions.');
    end
